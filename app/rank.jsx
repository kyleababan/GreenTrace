...
const calculateContributorAwards = (users, transactions) => {
  ...
  transactions.forEach((transaction) => {
    ...
    pointsByUser.set(
      transaction.userId,
      (pointsByUser.get(transaction.userId) || 0) +
...
export default function RankScreen() {
  ...
  useEffect(() => {
    const refreshRankings = async () => {
      try {
        ...
        const loadedUsers = usersSnapshot.docs.map((user) => ({
          id: user.id,
          ...user.data(),
        }));
        const loadedTransactions = transactionsSnapshot.docs.map(
          (transaction) => ({
            id: transaction.id,
            ...transaction.data(),
          }),
        );
        const awardsByUser = calculateContributorAwards(
          loadedUsers,
          loadedTransactions,
        );

        const usersWithAwards = loadedUsers.map((user) => {
          const existingBadges = Array.isArray(user.contributorBadges)
          ...
          if (missingBadges.length) {
            updateDoc(doc(db, "users", user.id), {
              ...
              console.error("Unable to award contributor badge:", error),
            );
          }

          return {
            ...user,
            contributorBadges: [...existingBadges, ...missingBadges],
          };
        });
  ...
  const rankings = useMemo(() => {
    ...
    const volunteerActivitiesByUser = volunteerPosts.reduce(
      (counts, activity) => {
    ...
    return users
      ...
      .filter((user) =>
        activeFilter === "monthly"
          ...
          : user.totalPoints > 0 ||
            user.cleanedReports > 0 ||
            user.volunteeredCount > 0,
      )
      .sort((first, second) =>
        activeFilter === "monthly"
        ...
        <View key={badge.id} style={styles.badgeWrapper}>
          ...
          {hoveredBadge === hoverKey && (
            <View style={styles.badgeTooltip}>
              ...
              {badge.periodLabel && (
                <Text style={styles.badgeTooltipPeriod}>
                  {badge.periodLabel}
                </Text>
              )}
        ...
        <View style={styles.filterContainer}>
          ...
          >
            <Ionicons
              ...
              size={17}
              color={activeFilter === "monthly" ? "#FFFFFF" : "#397A51"}
            />
            <Text
              style={[
                styles.filterText,
          ...
          >
            <Ionicons
              name="trophy-outline"
              size={17}
              color={activeFilter === "allTime" ? "#FFFFFF" : "#397A51"}
            />
            <Text
              style={[
                styles.filterText,
                activeFilter === "allTime" && styles.activeFilterText,
              ]}
            >
              All Time
            </Text>
              ...
              {activeFilter === "monthly" && (
                <View style={styles.countdownPill}>
                  ...
                  <Text style={styles.countdownText}>
                    {getMonthCountdown(countdownNow)}
                  </Text>
                </View>
              )}
            </View>
                ...
                <View style={styles.userDetails}>
                  <View style={styles.nameRow}>
                    ...
                    <View style={styles.contributorBadges}>
                      {renderContributorBadges(user)}
                    </View>
                  </View>
                  <Text style={styles.reportCount}>
                    {user.cleanedReports} cleaned{" "}
                    {user.cleanedReports === 1 ? "report" : "reports"}
                    {"  \u2022  "}
        ...
        >
          ...
          >
            <Pressable style={styles.badgeModal}>
              ...
              {selectedBadge?.periodLabel && (
                <Text style={styles.badgeModalPeriod}>
                  {selectedBadge.periodLabel}
                </Text>
              )}
